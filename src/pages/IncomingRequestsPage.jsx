...
import {
  ...
  MdArrowUpward, MdArrowDownward, MdArrowForward, MdInfoOutline, MdWarningAmber
} from 'react-icons/md';
...
function IncomingRequestsPage() {
  ...
  const canApproveRejectFulfill = useMemo(() => {
    return currentUser && allowedViewerRoles.includes(currentUser.role);
  }, [currentUser, allowedViewerRoles]);

  useEffect(() => {
    if (!isLoadingAuth) {
      if (!currentUser || !allowedViewerRoles.includes(currentUser.role)) {
  ...
  }, [currentUser, isLoadingAuth, navigate, fetchOrders, allowedViewerRoles]);
  ...
  const pendingOrders = useMemo(() => {
    ...
    if (sortConfig.key) {
      filtered.sort((a, b) => {
        ...
        if (sortConfig.key === 'request_date') { valA = new Date(valA); valB = new Date(valB); }
  ...
  const handleUpdateStatus = async (order, newStatus) => {
    ...
    switch (newStatus) {
      case 'Approved': titleText = `Approve Request for "${order.item_name}"?`; confirmText = 'Yes, Approve'; icon = 'question'; break;
      case 'Rejected': titleText = `Reject Request for "${order.item_name}"?`; confirmText = 'Yes, Reject'; showInput = true; inputOptions = { 'low_stock': 'Low Stock', 'not_available': 'Item Not Available', 'reason_misc': 'Other Reason (specify below)' }; icon = 'warning'; break;
      default: return;
    }
    const { value: formValues } = await Swal.fire({
      title: titleText, html: showInput ? ` <div class="flex flex-col gap-3 p-4"> <label class="block text-sm font-semibold mb-1 text-gray-700 dark:text-gray-300 text-left">Reason/Quantity:</label> <select id="swal-input1" class="swal2-input border rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-100"> ${Object.entries(inputOptions).map(([key, value]) => `<option value="${key}">${value}</option>`).join('')} </select> <input id="swal-input2" class="swal2-input border rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-100" placeholder="Details (e.g., specific reason)"> </div> ` : null,
      focusConfirm: false, showCancelButton: true, confirmButtonText: confirmText, preConfirm: showInput ? () => { const selectValue = document.getElementById('swal-input1').value; const textValue = document.getElementById('swal-input2').value; return { selectValue, textValue }; } : undefined,
    ...
    if (formValues) {
      let adminNotes = '';
      if (newStatus === 'Rejected') { adminNotes = formValues.selectValue + (formValues.textValue ? `: ${formValues.textValue}` : ''); }
      try {
        const token = localStorage.getItem('token');
        if (!token) throw new Error('No authentication token found.');
        const payload = { status: newStatus, fulfilled_quantity: 0, admin_notes: adminNotes, };
  ...
  if (isLoadingAuth || !canApproveRejectFulfill) {
    return (
      <div className={`flex h-screen items-center justify-center ${darkMode ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-700'}`}>
        {isLoadingAuth ? "Loading authentication..." : "Checking permissions..."}
      </div>
    );
  }

  // Helper to render the common page content structure
  const renderPageContent = (content) => (
    <div className={`flex-1 flex flex-col min-h-screen transition-colors duration-300 ${darkMode ? 'bg-slate-900' : 'bg-slate-100'}`}>
      {/* Header: CORRECTED positioning. It is fixed relative to the viewport.
          left-[250px] makes it start AFTER the fixed sidebar.
          right-0 makes it stretch to the viewport's right edge. */}
