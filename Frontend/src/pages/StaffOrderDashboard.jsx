...
const StaffOrderDashboard = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const itemsResponse = await axios.get(`${API_BASE_URL}/items/inventory`, { // Corrected interpolation here
                headers: { Authorization: `Bearer ${token}` }
            });
            // --- IMPORTANT FIX: Validate that itemsResponse.data is an array ---
            if (Array.isArray(itemsResponse.data)) {
                setItems(itemsResponse.data);
            } else {
                console.error("API returned non-array for inventory items:", itemsResponse.data);
                setItems([]); // Fallback to empty array
                Swal.fire({
                    ...swalThemeProps,
                    title: 'Data Error',
                    text: 'Received unexpected data format for inventory items.',
                    icon: 'error',
                });
            }
            ...
            // --- IMPORTANT FIX: Validate that historyResponse.data is an array ---
            if (Array.isArray(historyResponse.data)) {
                setOrderHistory(historyResponse.data);
            } else {
                console.error("API returned non-array for order history:", historyResponse.data);
                setOrderHistory([]); // Fallback to empty array
                Swal.fire({
                    ...swalThemeProps,
                    title: 'Data Error',
                    text: 'Received unexpected data format for order history.',
                    icon: 'error',
                });
            }
            ...
            setOrderHistory([]); // Ensure orderHistory is an array on error
        } finally {
            setIsLoading(false);
        }
    ...
    // This part is safe because items is guaranteed to be an array now
        ...
        try {
            ...
            const results = await Promise.allSettled(
                requests.map(requestData =>
                    axios.post(`${API_BASE_URL}/api/orders/request`, requestData, { // Corrected interpolation here
                        headers: { Authorization: `Bearer ${token}` }
                    })
                )
            );
    ...
    return (
        <Layout darkMode={darkMode}>
            {/* Header */}
            <header className={`flex items-center justify-between h-20 px-6 sm:px-8 fixed top-0 left-[250px] right-0 z-40 transition-colors duration-300 ${darkMode ? 'bg-slate-800/75 backdrop-blur-lg border-b border-slate-700' : 'bg-white/75 backdrop-blur-lg border-b border-slate-200'} shadow-sm`}>
